...
| Verificación de permisos en los interactores | A nivel interactor (`EnsurePermissionAsync`), como Identity — no solo a nivel UI | **Corrección respecto a una afirmación previa de este documento:** se verificó el código y hoy `EnsurePermissionAsync` solo lo llaman los interactores de Identity (Users/Roles); el resto de las features (Patients, MedicalVisit, MedicalAppointments, HealthInsurance, DynamicTemplates) no lo hacen todavía (es el "Plan 3" pendiente, documentado como fuera de alcance en la spec de Usuarios/Roles). Se decide que Professionals sí lo haga desde el día uno, por tratarse de datos de personal vinculados a acceso al sistema. Esto implica que `MedRec.Professionals.UseCases` referencia `MedRec.Identity.BusinessObjects` (solo interfaces: `IAuthorizationService` y `SystemPermissions`) — la primera dependencia cruzada entre features en la capa `UseCases` del proyecto. Es una excepción deliberada y acotada (un solo servicio + una clase de constantes), no un precedente general para acoplar features entre sí. |
